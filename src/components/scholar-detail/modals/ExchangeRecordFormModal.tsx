/**
 * 交往记录表单弹窗
 * 从 ScholarDetailPage.tsx:743-855 提取
 */
...
export function ExchangeRecordFormModal({
  ...
  const handleSubmit = () => {
    if (!form.date?.trim() || !form.title?.trim() || !form.organization?.trim()) {
      return;
    }
    onSubmit(form);
    onClose();
      ...
      >
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button
            ...
            disabled={
              !form.date?.trim() || !form.title?.trim() || !form.organization?.trim()
            }
